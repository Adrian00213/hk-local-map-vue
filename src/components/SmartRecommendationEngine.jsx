          ...
          <div className="flex-1 p-3 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <button
                ...
                className="w-8 h-8 rounded-xl bg-gradient-to-r from-yellow-500 to-yellow-600 flex items-center justify-center shrink-0 shadow-md active:scale-95 transition-transform"
              >
            ...
            <div className="flex items-center gap-2 mt-2">
              {place.rating && (
                <span className="inline-flex items-center gap-0.5 px-2 py-0.5 bg-yellow-100 text-yellow-700 text-xs font-medium rounded-full">
                  <Star className="w-3 h-3 text-yellow-600 fill-amber-500" />
                  {place.rating}
                </span>
              )}
            ...
            <div key={i} className="bg-white rounded-2xl border border-amber-100/30 overflow-hidden animate-pulse">
              <div className="flex">
                ...
                <div className="flex-1 p-3">
                  <div className="h-4 bg-yellow-200 rounded w-3/4 mb-2" />
    ...
    <div className="space-y-4 p-4">
      ...
      <div className={`rounded-2xl p-3 border ${usingGoogle ? 'bg-gradient-to-r from-emerald-50 to-teal-50 border-emerald-200/50' : 'bg-gradient-to-r from-amber-50 to-orange-50 border-amber-200/50'}`}>
        <div className="flex items-center gap-2">
          <Brain className={`w-4 h-4 ${usingGoogle ? 'text-emerald-500' : 'text-yellow-600'}`} />
          <span className={`text-xs font-medium ${usingGoogle ? 'text-emerald-700' : 'text-yellow-700'}`}>
            🧠 為你根據時間同位置智能推薦
          </span>
          <span className={`ml-auto text-[10px] ${usingGoogle ? 'text-emerald-600' : 'text-yellow-600'}`}>
      ...
      {recommendations.length === 0 && !loading && (
        <div className="text-center py-10">
          ...
          <button onClick={handleRefresh} className="mt-3 text-yellow-600 text-sm font-medium hover:text-yellow-700">
            重新整理 →
          </button>
        </div>
      )}
