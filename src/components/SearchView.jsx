        ...
        <div className="flex gap-2 mt-4 overflow-x-auto">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all active:scale-95 ${
              !cat ? 'bg-gradient-to-r from-yellow-500 to-yellow-600 text-white shadow-md' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200 active:bg-zinc-300'
            }`}
          >
            全部
          </button>
          {Object.entries(CATEGORY_LABELS).map(([k, v]) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all flex items-center gap-1.5 active:scale-95 ${
                cat === k ? 'bg-gradient-to-r from-yellow-500 to-yellow-600 text-white shadow-md' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200 active:bg-zinc-300'
              }`}
            >
                ...
                <div className="flex flex-wrap gap-2">
                  {suggestions.timeBased.map((s, i) => (
                    <button
                      ...
                      className="px-4 py-2.5 bg-yellow-100 border border-amber-200/50 rounded-xl text-sm font-medium text-yellow-700 flex items-center gap-1.5 hover:bg-yellow-200 active:scale-95 transition-all"
                    >
                      <span>{s.icon}</span>
                      {s.text}
                    </button>
                ...
                <div className="flex flex-wrap gap-2">
                  {suggestions.contextual.map((s, i) => (
                    <button
                      key={i}
                      onClick={() => handleSearch(s.text)}
                      className="px-4 py-2.5 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/50 rounded-xl text-sm font-medium text-yellow-700 flex items-center gap-1.5 hover:bg-yellow-200 active:scale-95 transition-all"
                    >
                      <span>{s.icon}</span>
                      {s.text}
                    </button>
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {['茶餐廳', '優惠', 'café', '商場', '港鐵站'].map((s, i) => (
                    <button
                      key={i}
                      onClick={() => handleSearch(s)}
                      className="px-3 py-1.5 bg-yellow-100 text-yellow-600 text-sm rounded-lg hover:bg-yellow-200 transition-colors"
                    >
                      {s}
                    </button>
                  ))}
                  ...
                  <div key={m.id} className="bg-white rounded-2xl p-4 shadow-sm border border-zinc-100/80 card-hover active:scale-[0.98] transition-transform">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="inline-block px-2.5 py-0.5 bg-yellow-100 text-yellow-600 text-xs font-medium rounded-full">
