...
export default function MarkerForm({ onClose, onLoginRequired }) {
  ...
  const categories = [
    ...
    { id: 'restaurants', label: '餐廳', icon: '🍜', color: 'from-yellow-600 to-yellow-500' },
      ...
      <div className="bg-white w-full sm:max-w-lg sm:rounded-3xl rounded-t-3xl max-h-[95vh] overflow-hidden animate-slide-up">
        ...
        <div className="p-5 space-y-5 overflow-y-auto max-h-[calc(95vh-180px)]">
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-4 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white font-bold rounded-xl shadow-lg shadow-amber-200 hover:shadow-xl transition-all active:scale-[0.98] disabled:opacity-50"
          >
            {loading ? (
