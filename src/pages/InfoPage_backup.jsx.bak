        ...
        {activeTab === 'food' && (
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {allRestaurantsFull.slice(0, displayedCount).map((r, i) => (
                <RestaurantCard key={r.name} restaurant={r} index={i} onLike={() => handleLike(r.name)} />
              ))}
              
              {/* Load More Button */}
              {allRestaurantsFull.length > displayedCount && (
                <button 
                  onClick={() => setDisplayedCount(prev => Math.min(prev + 50, allRestaurantsFull.length))}
                  className="w-full py-3 bg-orange-500 text-white rounded-2xl text-sm font-medium active:scale-98"
                >
                  載入更多 ({displayedCount}/{allRestaurantsFull.length})
                </button>
              )}
            </div>
          </div>
